...
function IndividualLesson() {
	...
	const handleCustom = (e) => {
		...
		setPackagePrice(Number(e.target.value) * ONE_LESSON_PRICE);
	};
	...
	const handleChange = useCallback((index, lesson, price) => {
		...
		return (e) => {
			...
			if (index === 4) {
				...
				localStorage.setItem('price', JSON.stringify(ONE_LESSON_PRICE));
			} else {
				...
				<form>
					<div className="row">
						...
						<div className="col-lg-4 offset-lg-4">
							...
							<p className="indiviual-lesson-info-text-2">Enter your preferred number of lessons</p>
						</div>
						<div className="col-lg-4 offset-lg-4">
							<label
								className={`indiviual-lesson my-container ${activeClass >= 4 ? 'special-active' : ''}`}
							>
								<input
									type="radio"
									value={0}
									name="lesson-name"
									onClick={handleChange(4, custom, ONE_LESSON_PRICE)}
									className="radio-buttons"
								/>
								<span className="checkmark"></span>
								<div className="lesson-count lesson-count-2">
									<div style={{ display: 'flex', alignItems: 'center', flexDirection: 'column' }}>
										<input
											className="lesson-number color-gray900"
											style={{ width: '50px' }}
											type="text"
											length={2}
											pattern="[0-9]{2}"
								...
								<div className="lesson-prices">
									<div style={{ width: 'max-content' }}>
										...
										<p className="per-lesson-price color-gray600">${650 / 10}/lesson</p>
									</div>
								</div>
							</label>
						</div>
