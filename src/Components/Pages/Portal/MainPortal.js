...
import PortalDrivingTest from './PortalDrivingTest';
import Toolbar from '../../Common/Toolbar';
import LargeHeading from '../../Common/LargeHeading';
import { useIntercom } from 'react-use-intercom';

function PortalPricing(props) {
	const { boot } = useIntercom();
	boot();

    const {open, setShowPricing, setDrivingTest} = props;
	...
	return (
		<section className="simple-bg border-radius-10">
						...
						<div className="pricing-box popular-tag">
							...
							>
								...
								<div className="align-center">
									{/* <Link
										to="/studentPortal/driving-test"
										state={{ heading_name: 'Book your first driving lesson' }}
										onClick={() => {handleFullPackage(); switchDrivingTest()}}
									>
										<div className="pricing-btn popular-btn">Select</div>
									</Link> */}
                                    <div className="pricing-btn popular-btn" onClick={() => {handleFullPackage(); switchDrivingTest()}}>Select</div>
								</div>
							</div>
						</div>
					</div>
...
function MainPortal(){
    ...
    const [showDrivingTest, setDrivingTest] = useState(false)

    console.log(showPricing)
     return(
        <React.Fragment>
            {showPricing === true ? <PortalPricing open={showPricing} setShowPricing={setShowPricing} setDrivingTest={setDrivingTest}/>: <></>}
            {showPricing === false ? <PortalDrivingTest open={showDrivingTest} setShowPricing={setShowPricing} setDrivingTest={setDrivingTest} /> : <></>}
        </React.Fragment>
    )
}
