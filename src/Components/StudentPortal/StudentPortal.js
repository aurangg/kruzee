...
import { studentLessons } from '../utils/APIs';
...
const StudentPortal = () => {
	...
	useEffect(() => {
		const getStudentLessons = async () => {
			...
			console.log('data student', data?.data[0]);
			...
			await axios
				.get(
					`${process.env.REACT_APP_BASE_URL}/api/student/getInstructorDetail?id=${data?.data[0]?.instructor}`
				)
				.then((res) => {
